...
struct tGroup
{
	...
	{
		tGroup::operator = (other);
	}
	const tGroup& operator = (const tGroup& other)
	{
		if (&other != this)
		{
			gid = other.gid;
...
Q_DECLARE_METATYPE(tGroup);

typedef QMap< QString, tGroup* >	tGroupMap;
typedef QMap< QString, tGroup >		tGroupRefMap;
typedef QVector< tGroup >			tGroupRefVector;
