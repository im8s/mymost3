...
BetCtlManager::BetCtlManager(QObject *parent)
	: QObject(parent)
{
	qRegisterMetaType<tGroup>("tGroup");
	qRegisterMetaType<tGroup>("tGroup&");

	qRegisterMetaType<tGroupRefMap>("tGroupRefMap");
	qRegisterMetaType<tGroupRefMap>("tGroupRefMap&");
	...
	m_grpLock = new QMutex();
	m_playerLock = new QMutex();
	m_bmLock = new QMutex();

	m_sm = new SMachine();
	//m_sm->setGStatus(GS_fetchservertime);
	...
	{
		tLoginParam& lp = m_loginParam;

		m_lqgClient->setParams(lp.strHost, lp.nPort);
...
void BetCtlManager::processLotteryResMsg(const LotteryResponse& msg)
{
	if (msg.has_lotstatus())
	{
		const pb::LotteryState& ls = msg.lotstatus();

		setLotteryState(ls.type(), ls.code());
	}
...
#if 1
void BetCtlManager::msgArrived(const QString& pid, const QString& strName, const QString& strMsg)
{
	qDebug() << "[BetCtlManager::msgArrived] pid = " << pid << ",strName = " << strName << ",strMsg = " << strMsg;

	if (m_bBetStartup)
	{
		/*if (m_strlstFilter.indexOf(pid) >= 0)
			return;*/

		if (strMsg.indexOf(ZN_STR("投注无效")) != -1  || strMsg.indexOf(ZN_STR("投注有效")) != -1)
		{
			//m_strlstFilter.append(pid);
			return;
		}

		putReceivedMsgToQueue(pid, strName, strMsg, 0);
	}
}
#else
void BetCtlManager::msgArrived(const QString& gid, const QString& pid, const QString& strName, const QString& strMsg)
{
	qDebug() << "[BetCtlManager::msgArrived] pid = " << pid << ",strName = " << strName << ",strMsg = " << strMsg;
...
void BetCtlManager::notifyGroupInfo(const QVariant& v)
{
	{
		clearAllGroups();

		const GroupInfoList* gilist = v.value<GroupInfoList*>();

		for (int k = 0; k < gilist->getCount(); ++k)
		{
			const GroupInfo* gi = gilist->getItem(k);

			tGroup p;
			{
				p.gid = gi->getGId();
				p.strName = gi->getName();
				p.type = gi->getType();
			}
			appendGroup(p);
		}
...
void BetCtlManager::notifyGroupCreated(const QVariant& v)
{
...
void BetCtlManager::notifyGroupDeleted(const QString& gid)
{
	removeGroup(gid);
...
void BetCtlManager::notifyMemberInfo(const QVariant& v)
{
	{
		clearAllPlayers();
		clearAllPScoreInfo();

		const PlayerInfoList* pilist = v.value<PlayerInfoList*>();

		for (int k = 0; k < pilist->getCount(); ++k)
		{
			const PlayerInfo* pi = pilist->getItem(k);

			tPlayer p;
			{
				p.pid = pi->getPId();
				p.strName = pi->getName();
				p.type = pi->getType();
			}
			appendPlayer(p);
...
void BetCtlManager::notifyMemberJoin(const QVariant& v)
{
	{
		const PlayerInfo* pi = v.value<PlayerInfo*>();
...
void BetCtlManager::notifyMemberLeave(const QString& pid)
{
	removePlayer(pid);
...
bool BetCtlManager::msgSend(const QString& strMsg)
{
	Q_EMIT sigMsgSend(m_strGrpId, strMsg);
...
void BetCtlManager::setPlayerList(const tPlayerRefMap& prColl, int type)
{
	...
	m_sm->doAction(GA_fetchplayerlistDone);
}
...
bool BetCtlManager::loadConfigFromFile(const QString& strXmlFilePath)
{
	...
	if (pConfig)
	{
		...
		{
			XMLElement *pServerParams = pConfig->FirstChildElement("serverParams");
			if (pServerParams)
			{
				tLoginParam& lp = m_loginParam;

				{
					XMLElement *pLotServer = pServerParams->FirstChildElement("lotServer");
					if (pLotServer)
					{
						lp.strHost = getAttrString(pLotServer, ZN_STR("ip"));
						lp.nPort = getAttrInt(pLotServer, ZN_STR("port"));

						lp.strUser = getAttrString(pLotServer, ZN_STR("user"));
						lp.strPass = getAttrString(pLotServer, ZN_STR("pass"));
					}
				}
			}
		}
