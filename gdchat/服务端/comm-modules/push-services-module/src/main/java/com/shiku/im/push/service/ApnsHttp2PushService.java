...
public class
ApnsHttp2PushService extends PushServiceUtils{
		...
		final String payload=builderPayloadOld(notice);
		 ...
		 if(PushEnvironment.VOIP!=env){
		   if(PushEnvironment.Pro==env)
			 notification=new ShikuApnsPushNotification(token,pushConfig.getAppStoreAppId(), payload,env,notice.getMessageId());
		   else 
			   notification=new ShikuApnsPushNotification(token,pushConfig.getBetaAppId(), payload,env,notice.getMessageId());
			   
		 }else
			 notification=new ShikuApnsPushNotification(token,pushConfig.getAppStoreAppId()+".voip", payload,env,notice.getMessageId());
