...
public class VIVOPushService extends PushServiceUtils{
	private static int appId=pushConfig.getVivo_appId();
	private static String appKey=pushConfig.getVivo_appKey();
	private static String appSecret=pushConfig.getVivo_appSecret();
		...
		if(null == sender){
			synchronized (appSecret){
				if(null==sender){
					...
					//发送鉴权请求
					Result token = appSender.getToken(appId, appKey);
					sender = new Sender(appSecret,token.getAuthToken());
					//设置连接池参数，可选项
	...
	**/ 
	public static Message buildMessage(String pushId,MsgNotice msgNotice) throws Exception {
		...
		if(msgNotice.getIsGroup())
			url="intent://"+pushConfig.getPackageName()+"/notification#Intent;scheme=sk;launchFlags=0x10000000;S.userId="+msgNotice.getRoomJid()+";end";
		else
			url="intent://"+pushConfig.getPackageName()+"/notification#Intent;scheme=sk;launchFlags=0x10000000;S.userId="+msgNotice.getFrom()+";end";
...
//                .extra("http://www.vivo.com", "vivo")
	...
	public static void noticeColumnMessagePush(String pushId, MsgNotice msgNotice) throws Exception{
		try {
			Result resultMessage = getSender().sendSingle(buildMessage(pushId,msgNotice));
	...
	**/ 
	public static void allSend(String pushId,MsgNotice msgNotice) throws Exception {
        ...
        Message allSend = new Message.Builder()
                ...
                .build();//构建要全量推送的消息体
        Result results = getSender().sendToAll(allSend);//发送全量推送消息请求
...
//        result.getTaskId();//如请求发送成功，将获得该条消息的任务编号，即taskId
	...
	public static String saveListPayload(MsgNotice msgNotice) throws Exception {
		...
		if(null == msgNotice.getObjectId())
			...
			url="intent://"+pushConfig.getPackageName()+"/notification#Intent;scheme=sk;launchFlags=0x10000000;S.url="+URLEncoder.encode(msgNotice.getObjectId(), "UTF-8")+";end";
...
//        regid.add(saveListPayload());// 2 - 1000
		try {
			TargetMessage targetMessage = new TargetMessage.Builder()
					...
					.requestId(StringUtil.randomUUID())
					.build();//构建批量推送的消息体
			Result resultTarget = getSender().sendToList(targetMessage);//批量推送给用户
