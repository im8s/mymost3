...
public class OPPOPushService extends PushServiceUtils {
	private static String appKey = pushConfig.getOppo_appKey();
	private static String masterSecret = pushConfig.getOppo_masterSecret();
	...
	**/ 
	private static Notification getNotification(MsgNotice msgNotice) {
		...
		if(msgNotice.getIsGroup())
			url="sk://"+pushConfig.getPackageName()+"/notification?userId="+msgNotice.getRoomJid();
		else
			url="sk://"+pushConfig.getPackageName()+"/notification?userId="+msgNotice.getFrom();
...
//		notification.setActionParameters("{\"key1\":\"value1\",\"key2\":\"value2\"}");
	...
	public static void broadcastMessage(MsgNotice msgNotice) throws Exception {
		...
		if(null == msgNotice.getObjectId())
			url="sk://"+pushConfig.getPackageName()+"/notification";
		else
			url="sk://"+pushConfig.getPackageName()+"/notification?url="+URLEncoder.encode(msgNotice.getObjectId(), "UTF-8");
		try {
