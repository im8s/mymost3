...
layui.use(['form','layer','laydate','table','laytpl'],function(){
    ...
    var tableInUser = table.render({
        ...
        ,cols: [[ //表头
            ...
            ,{field: 'isOpenResetPwd', title: '是否开启端到端加密',sort:'true', width:100}
            ...
            ,{field: 'invitedUserId', title: '被邀请自用户Id',sort:'true', width:170}
            ...
            ,{fixed: 'right', width: '74%',title:"操作", align:'left', toolbar: '#userListBar'}
        ]]
    ...
    table.on('tool(user_list)', function(obj){
        ...
        if(layEvent === 'delete'){ //删除
            ...
            if (data.userId <= 10000 || 1 != data.isOpenResetPwd) {
                layer.msg("该用户不支持重置密码");
                return;
            }
        ...
        }else if(layEvent==='ipInfo'){// ip信息
            if (!data.ipInfo || data.ipInfo.length == 0) {
                layer.msg("暂无ip信息");
                return;
            }
            nickName = Common.filterHtmlData(data.nickname);
            $(".disUserName").val(nickName);
            $(".disUserName").empty();
            $(".disUserName").append( Common.getValueForElement(".disUserName") );// ip信息中的小标题用户昵称
            var ipInfomations = table.render({

                elem: '#ipInfomation_table'
                ,id: 'ipInfomation_table'
                ,data: data.ipInfo
                ,page: true
                ,curr: 0
                ,limit:Common.limit
                ...
                ,cols: [[ //表头
                    ...
                    ,{field: 'ipLocation', title: '地理位置', width:500}
                ]]
                ,done:function(res, curr, count){
                    checkRequst(res);
                    $("#user_table").hide();
                    $("#ipInfomation").show();
                    $(".user_btn_div").hide();
                    var pageIndex = ipInfomations.config.page.curr;//获取当前页码
                    var resCount = res.count;// 获取table总条数
                    currentCount = resCount;
                    currentPageIndex = pageIndex;
                }
        ...
        }else if(layEvent==='contacts'){// ip信息
            localStorage.setItem("currClickUser", data.userId);
            nickName = Common.filterHtmlData(data.nickname);
            $(".disUserName").val(nickName);
            $(".disUserName").empty();
            $(".disUserName").append( Common.getValueForElement(".disUserName") );// ip信息中的小标题用户昵称
            var contactsTable = table.render({

                elem: '#contacts_table'
                ,toolbar: '#contactsToolbar'
                ,id: 'contacts_table'
                ,url:request("/console/addressBookList")+"&userId="+data.userId
                ,page: true
                ,curr: 0
                ,limit:Common.limit
                ,limits:Common.limits
                ,groups: 7
                ,cols: [[ //表头
                    {field: 'toRemarkName', title: '姓名', width:300}
                    ,{field: 'toTelephone', title: '电话', width:500}
                ]]
                ,done:function(res, curr, count){
                    checkRequst(res);
                    $("#user_table").hide();
                    $("#contacts").show();
                    $(".user_btn_div").hide();
                    var pageIndex = contactsTable.config.page.curr;//获取当前页码
                    var resCount = res.count;// 获取table总条数
                    currentCount = resCount;
                    currentPageIndex = pageIndex;
                }
    ...
    /*table.on('toolbar(contacts_table)', function (obj) {
        if (obj.event === 'exportContacts') {
            exportContacts();
        }
    });*/
    ...
    table.on('tool(myBill_table)', function(obj){
        ...
        if(layEvent === 'billDetails'){
            ...
            if(4 == data.type || 5 == data.type || 6 == data.type){
                ...
                tableBillInfo = table.render({
                    ...
                    ,cols: [[ //表头
                        ...
                        ,{field: 'type', title: '类型',sort: true, width:150, templet : function (d) {
                                var statusMsg;
                                (d.type == 3 ? statusMsg = "后台充值" : (d.type == 10) ? statusMsg = "付款码付款": (d.type == 11) ?
                ...
                tableBillInfo = table.render({
                    ...
                    ,groups: 7
                    ,cols: [[ //表头
                        {field: 'id', title: '礼物账单id',sort: true, width:220}
                        ,{field: 'money', title: '价格',sort: true, width:220}
                        ,{field: 'serviceCharge', title: '手续费',sort: true, width:220}
                        ,{field: 'operationAmount', title: '实际收入',sort: true, width:220}
                        ,{field: 'userId', title: '赠送人Id',sort: true, width:220,templet:function (d) {
                                if(14 == data.type)
                                    return d.userId;
                                else
                                    return d.toUserId;
                            }}
                        ,{field: 'toUserId', title: '接收人Id',sort: true, width:220,templet:function (d) {
                                if(14 == data.type)
                                    return d.toUserId;
                                else
                                    return d.userId;
            ...
            }else if(7 == data.type || 8 == data.type || 9 == data.type){
                var titleName;
                ...
                tableBillInfo = table.render({
                    ...
                    , cols: [[ //表头
                        ...
                        , {
                            field: 'userId', title: '转账用户Id', sort: true, width: 120, templet: function (d) {
                                if (7 == data.type)
                                    return d.userId;
                                else
                                    return d.toUserId;
                        ...
                        , {
                            field: 'userName', title: titleName, sort: true, width: 150, templet: function (d) {
...
$(".export_rand_user").on("click",function(){
    ...
    layui.layer.open({
        ...
        ,content:  '<form class="layui-form" method="post" action='+requestUrl+'>'
            ...
            + 		'<label class="layui-form-label" style="width:80%">导出由系统自动生成的随机用户</label>'
...
var User={
    ...
    balanceTotal:function(){
        ...
        $(".layui-table-box td[data-field='balance'] .layui-table-cell").each(function (i,item) {
            //console.log(balanceTotal);
            var num = Number($(item).text());
            if(null!=num&&!isNaN(num)&&0!=num){
                balanceTotal=balanceTotal+num;
            }

        });
         balanceTotal=balanceTotal.toFixed(2);
         console.log("balanceTotal ===> "+balanceTotal);
        $(".balanceTotal").empty().text(balanceTotal);
    ...
    commit_addUser:function(){
        ...
        if(0 == $("#userId").val() && Common.isNil($("#password").val())){
        ...
        if(acc.match(pattern) || chains.test(acc)){
            layui.layer.alert("通讯号只支持英文与数字，不支持特殊符号与中文！");
            return;
        }

        if("" == $("#isPermit").val() || null == $("#isPermit").val()){
            layui.layer.alert("是否设置为商务号");
            return;
        }
        if (InviteCodeContest == 1){
            if($("#InviteCode").val()==""){
                layui.layer.alert("请输入邀请码！");
                return;
            }
        }
        ...
        Common.invoke({
            ...
            success:function(result){
                ...
                if(result.data!=null){
                    ...
                    $('#isPermit').val(result.data.permitUserType);
    ...
    // 导出通讯录明细
    exportContacts : function(userId){
        var requestUrl = request('/console/exportExcelByAddressBook')+"&userId="+ Common.filterHtmlData(localStorage.getItem("currClickUser"));
        ...
        layui.layer.open({
            ...
            ,content:  '<form class="layui-form" method="post" action='+requestUrl+'>'
                + '<div class="layui-form-item">'
                + 	'<div class="layui-inline">'
                + 		'<label class="layui-form-label" style="width: 90%;margin-top: 20px;text-align: center" >导出通讯录</label>'
                +	'</div>'
                + '</div>'
                +  '<button id="exportContacts_submit"  class="layui-btn" type="submit" lay-submit="" style="display:none">导出</button>'
                +'</from>'
            ,success: function(index, layero){
                layui.form.render();
            }
            ,yes: function(index, layero){
                $("#exportContacts_submit").click();
                layui.layer.close(index); //关闭弹框
            }
            ,btn2: function(index, layero){
                //按钮【取消】的回调
    ...
    exportExcelPhone : function(){
        ...
        layui.layer.open({
            ...
            ,content:  '<form class="layui-form" method="post" action='+requestUrl+'>'
                ...
                + 		'<label class="layui-form-label" style="width: 90%;margin-top: 20px;text-align: center" >导出当前筛选的手机号</label>'
                +	'</div>'
                + '</div>'
                +  '<button id="exportFriends_submit"  class="layui-btn" type="submit" lay-submit="" style="display:none">导出</button>'
                +'</from>'
