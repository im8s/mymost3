...
var Gift={
	...
	findGiftList:function(){
		...
		$.ajax({
			...
			url:request('/console/giftList'),
			data:{
				name: Common.filterHtmlData("#giftName")
			},
			dataType:'json',
			async:false,
			success:function(result){
				...
				if(result.data.pageData!=null){
					$("#pageCount").val( Common.filterHtmlData(result.data.allPageCount) );
					for(var i=0;i<result.data.pageData.length;i++){
						htm+="<tr><td>"+Common.filterHtmlData(result.data.pageData[i].name) +"</td><td>"+ Common.filterHtmlData(result.data.pageData[i].photo)
						+"</td><td>"+Common.filterHtmlData(result.data.pageData[i].price)+"</td><td>"+Common.filterHtmlData(result.data.pageData[i].type)
						+"</td><td><button onclick='Gift.deleteGift(\""+result.data.pageData[i].giftId+"\")' class='layui-btn layui-btn-danger layui-btn-xs delete'>删除</button></td></tr>";
					}
					$("#giftList_table").empty();
