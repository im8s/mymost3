...
var App={
	...
	lookInfo:function(id){
		...
		myFn.invoke({
			...
			success:function(result){
				...
				$("#isAuthShareOperate").append(result.data.isAuthShare==0||result.data.isAuthShare==-1?"<a onclick='App.applicationShare(\""+result.data.id+"\")' style='color:#3292ff;cursor:pointer;'>申请开通</a>":"<a onclick='' style='cursor:pointer;'>--<a>");
				$("#isAuthLoginOperate").empty();
				$("#isAuthLoginOperate").append(result.data.isAuthLogin==0||result.data.isAuthLogin==-1?"<a onclick='App.applicationLogin(\""+result.data.id+"\")' style='color:#3292ff;cursor:pointer;'>申请开通</a>":"<a onclick='' style='cursor:pointer;'>--</a>");
				$("#isAuthPayOperate").empty();
				$("#isAuthPayOperate").append(result.data.isAuthPay==0||result.data.isAuthPay==-1?"<a onclick='App.applicationPay(\""+result.data.id+"\")' style='color:#3292ff;cursor:pointer;'>申请开通</a>":"<a onclick='' style='cursor:pointer;'>--</a>");
				$("#isGroupHelperOperate").empty();
				$("#isGroupHelperOperate").append(result.data.isGroupHelper==0||result.data.isGroupHelper==-1?"<a onclick='App.applicationGroupHelper(\""+result.data.id+"\")' style='color:#3292ff;cursor:pointer;'>申请开通</a>":result.data.isGroupHelper==2?"<a onclick='' style='cursor:pointer;'>--</a>":"<a onclick='App.groupHelperList(\""+result.data.id+"\")' style='cursor:pointer;'>添加群助手</a>");
			}
		})
		$("#applist").hide();
		$("#createApp_div").hide();
	...
	confirmApplicationPay:function(){
		myFn.invoke({
			...
			success:function(result){
				layui.layer.alert("申请成功，待审核");
				App.lookInfo($("#applicationPayId").val());
			}
		})
	},
	// 返回上一级
	...
	confirmIsGroupHelper:function(){
		...
		if(!myFn.isNil($("#helDeveloper").val())){
			param.developer = $("#helDeveloper").val();
		}else{
			layui.layer.alert("请输入开发者");
			return;
		}
		if($("#HelperType").val()!=0){
		...
		if(param.type==2){
			...
			if(!myFn.isNil($("#appPackName").val())){
				param.appPackName = $("#appPackName").val();
			}
			if(!myFn.isNil($("#callBackClassName").val())){
				param.callBackClassName = $("#callBackClassName").val();
			}
			if(!myFn.isNil($("#iosUrlScheme").val())){
		...
		myFn.invoke({
			...
			success:function(result){
				...
				App.groupHelperList(param.openAppId);
			}
		})
	},
		...
		myFn.invoke({
			...
			success:function(result){
				if(result.resultCode==1){
					...
					App.groupHelperList($("#addHelper_openAppId").val());
				}else{
