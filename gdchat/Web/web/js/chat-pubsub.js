...
var  PUBSUB={
	...
	onMessage:function(elem){
		...
		for (var i = 0; i < msgArr.length; i++){
			...
			if(!myFn.isNil(DataUtils.getMessage(messageId))){
			 ...
			 bodyElem=message.getElementsByTagName('body')[0];
			 body=Strophe.getText(bodyElem).replace(/&quot;/gm, '"');
			 msg = JSON.parse(body.replace(/&quot;/gm, '"'));
	...
	//订阅节点
	_XEP_0060_032:function(node){
		var msgId=myFn.randomUUID();
		var elem = $iq({
			from: WEBIM.userIdStr,
			to : this.jid,
			id : msgId,
			type:"set"
		}).c("pubsub", {
...
*/
    ...
    init: function(conn) {
       ...
       // this._connection=conn.xmlInput
        /*
