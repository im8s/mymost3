...
namespace WinFrmTalk.Controls.CustomControls
{
    public partial class ShowMsgPanel : UserControl, IShowMsgPanel
    {
        ...
        public void SendMsg(string strSend)
        {
            if (ChooseTarget == null || string.IsNullOrEmpty(ChooseTarget.UserId))
                return;
                ...
                Thread sendThread = new Thread(() =>
                {
                    try
                    {
                        if (!string.IsNullOrEmpty(strSend))
                        {
                            ...
                            if (!UIUtils.IsNull(atFriends))
                            {
                                atFriends = atFriends.FindIndex(f => f.UserId.Equals("allRoomMember")) > -1 ? new List<Friend>() : atFriends;
                                txt_msg = ShiKuManager.SendAtMessage(fdSend, atFriends, strSend, false);
                            }
                            else
                                txt_msg = ShiKuManager.SendTextMessage(fdSend, strSend, false);
            ...
            {
                LogHelper.log.Error("----发送消息出错，方法（btnSend_Click） : " + ex.Message, ex);
            }
...
ShiKuManager.SendMessage(msg);                                                }
        ...
        private void TxtSend_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.V)
            {
                ...
                {
                    LogHelper.log.Error("-----------粘贴图片到文本框时出错，txtSend_KeyUp：", ex);
                }
            }
        ...
        private void TxtSend_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.Control && e.KeyCode == Keys.V)
            {
                ...
                {
                    LogHelper.log.Error("-----------粘贴数据到文本框时出错，txtSend_KeyDown：", ex);
                }
            }
        }
        #endregion
