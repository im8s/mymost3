...
+(void)sortAndGroup:(NSArray*)objectArray
    ...
    if (key == nil) {
        ...
        Ivar *ivars;
        if ([obj isKindOfClass:[JXUserObject class]] && ![obj isKindOfClass:[JXDevice class]]) {
            ivars = class_copyIvarList(obj.superclass, &outCount);
        }else if([obj isKindOfClass:[JXDevice class]]) {
            ivars = class_copyIvarList([obj.superclass superclass], &outCount);
        }else {
            ivars = class_copyIvarList(obj.class, &outCount);
        }
...
+(BMChineseSortModel*)getModelWithObj:(id)obj key:(NSString*)key{
    ...
    if (!key) {
        ...
        if ([obj isKindOfClass:[JXUserObject class]] || [obj isKindOfClass:[JXDevice class]]) {
            model.string = [(JXUserObject *)obj remarkName].length > 0 ? [(JXUserObject *)obj remarkName] : [(JXUserObject *)obj userNickname];
        }else if([obj isKindOfClass:[memberData class]]){
            model.string = [(memberData *)obj lordRemarkName].length > 0 ? [(memberData *)obj lordRemarkName] : [(memberData *)obj userNickName];
        }else if([obj isKindOfClass:[JXAddressBook class]]) {
            model.string = [(JXAddressBook *)obj addressBookName];
        }else {
            model.string = [obj valueForKey:key];
        }
    }
    //提出空白字符
