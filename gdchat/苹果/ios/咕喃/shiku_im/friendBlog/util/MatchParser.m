...
//        dic =[NSDictionary dictionaryWithContentsOfFile:path];
        ...
        for (NSInteger i = 0; i < g_constant.emojiArray.count; i++) {
            NSDictionary * emoji = g_constant.emojiArray[i];
            NSString *EName = [emoji objectForKey:@"english"];
            NSString *totalEname = [NSString stringWithFormat:@"[%@]",EName];
            NSString *filename = [emoji objectForKey:@"filename"];
            [dict setValue:filename forKey:totalEname];
        }
...
+ (NSString *)emojiExist:(NSString *)key map:(NSDictionary *)map{
    NSArray * keys=[map allKeys];
    for (int i = 0; i < keys.count; i++) {
        if ([keys[i] isEqualToString:key]) {
            return map[keys[i]];
        }
    }
    return nil;
...
//        NSString * icon=[MatchParser faceKeyForValue:string map:[MatchParser getFaceMap]];
        NSString *icon = [MatchParser emojiExist:string map:[MatchParser getFaceMap]];
...
#pragma -mark 获取内容行数 以及 一行时内容的宽度
    // 获取内容行数 以及 一行时，内容的宽度
...
//        NSRange imgLocation =[[nextImage objectForKey:MatchParserLocation] rangeValue];
        NSRange imgLocation = NSMakeRange([[nextImage objectForKey:MatchParserLocation] intValue], 1);
        int lineIndex = 0;
...
//                CTRunRef run = (__bridge CTRunRef)runs[j];
                ...
                if ( runRange.location==imgLocation.location) { //7
                    ...
                    if (imgIndex < [self.images count]) {
                        ...
                        imgLocation = NSMakeRange([[nextImage objectForKey:MatchParserLocation] intValue], 1);
                    }else{
                        lineIndex = (int)[lines count];
                        break;
                    }
                }
            }
