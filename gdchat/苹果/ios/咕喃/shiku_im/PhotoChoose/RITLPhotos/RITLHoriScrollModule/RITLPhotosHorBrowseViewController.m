...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    self.backHandler(self.isEdit,self.asset);
}
...
- (void)editImage {
    ...
    KKImageEditorViewController *editor = [[KKImageEditorViewController alloc] initWithImage:self.cell.imageView.image delegate:self];
    editor.asset = [self.dataSource assetAtIndexPath:[NSIndexPath indexPathForRow:self.cellRow inSection:0]];
...
- (void)imageDidFinishEdittingWithImage:(UIImage *)image asset:(PHAsset *)asset
{
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        if (error) {
            NSLog(@"%@",@"保存失败");
            [JXMyTools showTipView:Localized(@"JX_PictureEditingFailed")];
        } else {
            NSLog(@"%@",@"保存成功");
            dispatch_async(dispatch_get_main_queue(), ^{
