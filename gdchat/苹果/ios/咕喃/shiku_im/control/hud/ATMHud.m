...
- (void)showWithouAnimation:(UIView*)parent{
    [self setActivityStyle:UIActivityIndicatorViewStyleWhite];
    [parent addSubview:self.view];
    isShowing = YES;
    [__view calculate];
    [__view showWithoutAnimation];
}
...
- (void)startWithClearColor {
    self.alpha = 0;
    [self setActivity:YES];
    [self setActivityStyle:UIActivityIndicatorViewStyleGray];
    [self show:[UIApplication sharedApplication].keyWindow];
}
...
- (void)startWithouAnimation{
    self.alpha = 0.5;
    [self setActivity:YES];
    [self showWithouAnimation:[UIApplication sharedApplication].keyWindow];
}

- (void)startWithouAnimation:(NSString*)s{
    self.alpha = 0.5;
    [self setCaption:s];
    [self setActivity:YES];
    [self showWithouAnimation:[UIApplication sharedApplication].keyWindow];
...
- (void)startWithouAnimation:(NSString*)s delay:(int)delay{
    self.alpha = 0.5;
    [self setCaption:s];
    [self showWithouAnimation:[UIApplication sharedApplication].keyWindow];
    [self hideAfter:delay];
